...
export class UniformMap extends Map {
   ...
   * @see ROT.Map#create
   */
  create(callback:(x:number, y:number, value:number) => void):UniformMap
  {
    let t1 = Date.now();
    while (1)
    {
   ...
   * @returns {boolean} success Was this attempt successfull?
   */
  _generateCorridors()
  {
    let cnt = 0;
    while (cnt < this._corridorAttempts)
    {
      ...
      while (1)
      {
        ...
        let connected = this._connected.random();
  ...
  _connectRooms(room1, room2)
  {
    ...
    let start = this._placeInWall(room1, dirIndex1);
    ...
    { /* need to switch target wall (L-like) */
      ...
      let end = this._placeInWall(room2, dirIndex2);
      if (!end)
      {
        return false;
      }
